...
export function PartyPurchasesClient({
  data,
  pagination,
  query,
}: {
  ...
  const pushTableQuery = usePrefixedTableUrlQuery(PARTY_PURCHASES_QUERY_PREFIX);

  const [paymentStatus, setPaymentStatus] = useState<PartyPurchasePaymentStatusFilter>(
    query.paymentStatus,
  );
  const [from, setFrom] = useState(query.from);
  const [to, setTo] = useState(query.to);

  const columns: Array<ColumnDef<PartyPurchaseRowDto>> = useMemo(
    () => [
      {
        header: "Folio",
        field: "supplierFolio",
  ...
  const actions: Array<TableActionDef<PartyPurchaseRowDto>> = [
    {
      type: "details",
      label: "Detalles",
      tooltip: "Ver compra",
  ...
  const updateFilters = (next: {
    ...
    if (nextPaymentStatus === "all") {
      params.delete(PARTY_PURCHASES_QUERY_KEYS.paymentStatus);
    } else {
      params.set(PARTY_PURCHASES_QUERY_KEYS.paymentStatus, nextPaymentStatus);
    }

    setParam(params, PARTY_PURCHASES_QUERY_KEYS.from, nextFrom);
    setParam(params, PARTY_PURCHASES_QUERY_KEYS.to, nextTo);

    params.set(PARTY_PURCHASES_QUERY_KEYS.page, "1");
    ...
    <div className="space-y-4">
      <div className="rounded-box border border-base-300 bg-base-200/70 p-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h3 className="font-semibold">Compras relacionadas</h3>
            <p className="text-sm opacity-70">
              Compras vinculadas al contacto seleccionado. Se pueden filtrar por
              fecha de compra o estado de pago.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)_auto]">
          <div className="form-control">
            ...
            <select
              ...
              value={paymentStatus}
              onChange={(e) => {
                const next = e.target.value as PartyPurchasePaymentStatusFilter;
            ...
            <p className="mt-1 text-xs opacity-60">
              Muestra todas las compras o sólo las que ya están pagadas.
            </p>
          </div>
          ...
          <div className="rounded-box border border-base-300 bg-base-100 p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-xs opacity-60">Por fecha de compra</span>
            </div>
      ...
      <GenericPaginatedTable<PartyPurchaseRowDto>
        key={queryKey}
        data={data}
        columns={columns}
        actions={actions}
        ...
        onAction={(e) => {
          switch (e.type) {
            ...
            case "pdf":
              window.open(
                routes.supplierPurchases.pdf(e.row.id),
                "_blank",
                "noopener,noreferrer",
              );
              break;
